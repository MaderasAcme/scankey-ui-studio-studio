...
const postAnalyze = async (frontFile: any, backFile: any) => {
  ...
  // Validaciones con mensajes EXACTOS solicitados
  if (!base) throw new Error('Falta BASE_URL (pégala en Settings)');
  ...
  // Campos exactos: front y back + image_front y image_back para compatibilidad con el motor
  ...
  // Lead Engineer Fix: Implementación de AbortController + Timeout real (30s)
  ...
  try {
    ...
    // El Gateway devuelve el objeto de análisis directamente
...
export const analyzeKey = async (front: any, back: any, onAttempt: any) => {
  try {
    ...
    if (status === 401 || status === 403) throw e; // No reintentar en fallos de Auth
...
/**
 * Lead Engineer Fix: Implementación de sendFeedback con cola local de respaldo.
 * Envía feedback al servidor o lo encola localmente si falla la red.
 */
...
/**
 * Intenta vaciar la cola local de feedback acumulada.
 */
